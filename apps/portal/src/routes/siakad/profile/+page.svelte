<script lang="ts">
	...
	let isLecturer = $derived(data.isLecturer || false);

	let activeTab = $state('general');
	let lecturerTab = $state('identity');
	let photoDialogOpen = $state(false);

	// Edit Mode States — Student
	...
	// Edit Mode States — Lecturer
	let editLecturerGeneral = $state(false);
	let editLecturerContact = $state(false);
	let editLecturerEducation = $state(false);
</script>

<div class="space-y-6">
	<div>
		<h2 class="text-3xl font-bold tracking-tight">
			{isLecturer ? 'Profil Dosen' : 'Profil Mahasiswa'}
		</h2>
		<p class="text-muted-foreground">
			{isLecturer
				? 'Kelola informasi data diri, kepegawaian, dan akademik.'
				: 'Kelola informasi lengkap data diri, akademik, dan keluarga.'}
		</p>
	</div>
	...
	{#if isLecturer}
						...
						<Dialog bind:open={photoDialogOpen}>
							...
							<DialogContent>
								...
								<form
									...
									use:enhance={() => {
										return async ({ result }) => {
											if (result.type === 'success') photoDialogOpen = false;
						...
						<div class="space-y-4">
							<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
								...
								<div class="space-y-2">
									<Label>NIDN</Label>
									<Input
										value={profile.nidn || profile.lecturer_number || '-'}
										disabled
										class="bg-muted"
									/>
								</div>
								<div class="space-y-2">
									...
									<Input value={profile.study_program || '-'} disabled class="bg-muted" />
								</div>
								<div class="space-y-2">
									<Label>Status</Label>
									<Input value={profile.status || 'Aktif'} disabled class="bg-muted capitalize" />
								</div>
								<div class="space-y-2">
									<Label>Jabatan Fungsional</Label>
									<Input value={profile.functional_position || '-'} disabled class="bg-muted" />
								</div>
								<div class="space-y-2">
									<Label>Golongan</Label>
									<Input value={profile.rank || '-'} disabled class="bg-muted" />
								</div>
								<div class="space-y-2">
									<Label>Status Ikatan Kerja</Label>
									<Input value={profile.employment_status || '-'} disabled class="bg-muted" />
								</div>
								<div class="space-y-2">
		...
		<!-- LECTURER TABS -->
		<Tabs value={lecturerTab} onValueChange={(v: string) => (lecturerTab = v)} class="space-y-4">
			<TabsList class="grid h-auto w-full grid-cols-2 lg:grid-cols-4">
				<TabsTrigger value="identity">Identitas</TabsTrigger>
				<TabsTrigger value="education">Pendidikan</TabsTrigger>
				<TabsTrigger value="expertise">Bidang Keahlian</TabsTrigger>
				...
				<Card>
					<CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
						<div class="space-y-1">
							...
							<CardDescription>Data diri dosen</CardDescription>
						</div>
						{#if !editLecturerGeneral}
							<Button variant="ghost" size="sm" onclick={() => (editLecturerGeneral = true)}
								>Edit</Button
							>
						{/if}
					</CardHeader>
					<CardContent>
						{#if !editLecturerGeneral}
							<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
								...
								<div>
									<p class="text-sm font-medium text-muted-foreground">NIDN</p>
									<p>{profile.nidn || profile.lecturer_number || '-'}</p>
								</div>
								<div>
								...
								<div>
									<p class="text-sm font-medium text-muted-foreground">NPWP</p>
									<p>{profile.npwp || '-'}</p>
								</div>
							</div>
						{:else}
							<form
								...
								use:enhance={() => {
									return async ({ result }) => {
										if (result.type === 'success') editLecturerGeneral = false;
										await applyAction(result);
									};
								}}
								class="space-y-4"
							>
								<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
									<div class="space-y-2">
										<Label>Nama Lengkap</Label><Input value={profile.name} disabled />
									</div>
									<div class="space-y-2">
									...
									<div class="space-y-2">
										<Label for="gender">Jenis Kelamin</Label><Input
											id="gender"
											name="gender"
									...
									<div class="space-y-2">
										<Label for="religion">Agama</Label><Input
											id="religion"
											name="religion"
									...
									<div class="space-y-2">
										<Label for="place_of_birth">Tempat Lahir</Label><Input
											id="place_of_birth"
											name="place_of_birth"
									...
									<div class="space-y-2">
										<Label for="date_of_birth">Tanggal Lahir</Label><Input
											id="date_of_birth"
											name="date_of_birth"
											type="date"
									...
									<div class="space-y-2">
										<Label for="nik">NIK</Label><Input id="nik" name="nik" value={profile.nik} />
									</div>
									<div class="space-y-2">
								...
								<div class="flex justify-end gap-2">
									<Button
										type="button"
										variant="outline"
										onclick={() => (editLecturerGeneral = false)}>Batal</Button
									>
			...
			<!-- EDUCATION -->
			<TabsContent value="education" class="space-y-4">
				<Card>
					<CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
						<div class="space-y-1">
							<CardTitle>Riwayat Pendidikan</CardTitle>
							<CardDescription>Pendidikan terakhir dan sertifikasi</CardDescription>
						</div>
						{#if !editLecturerEducation}
							<Button variant="ghost" size="sm" onclick={() => (editLecturerEducation = true)}
								>Edit</Button
							>
						{/if}
					</CardHeader>
					<CardContent>
						{#if !editLecturerEducation}
							<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
								<div>
									<p class="text-sm font-medium text-muted-foreground">Pendidikan Terakhir</p>
									<p>{profile.last_education || '-'}</p>
								</div>
								<div>
									<p class="text-sm font-medium text-muted-foreground">Universitas</p>
									<p>{profile.alma_mater || '-'}</p>
								</div>
								<div>
									<p class="text-sm font-medium text-muted-foreground">Gelar</p>
									<p>{profile.degree_title || '-'}</p>
								</div>
								<div>
									<p class="text-sm font-medium text-muted-foreground">Bidang Studi</p>
									<p>{profile.field_of_study || '-'}</p>
								</div>
								<div>
									<p class="text-sm font-medium text-muted-foreground">Tahun Lulus</p>
									<p>{profile.education_grad_year || '-'}</p>
								</div>
								<div>
									<p class="text-sm font-medium text-muted-foreground">No. Sertifikasi Dosen</p>
									<p>{profile.certification_number || '-'}</p>
								</div>
							</div>
						{:else}
							<form
								...
								use:enhance={() => {
									return async ({ result }) => {
										if (result.type === 'success') editLecturerEducation = false;
										await applyAction(result);
									};
								}}
								class="space-y-4"
							>
								<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
									<div class="space-y-2">
										<Label for="last_education">Pendidikan Terakhir</Label><Input
											id="last_education"
											name="last_education"
									...
									<div class="space-y-2">
										<Label for="alma_mater">Universitas</Label><Input
											id="alma_mater"
											name="alma_mater"
											value={profile.alma_mater}
										/>
									</div>
									<div class="space-y-2">
										<Label for="degree_title">Gelar</Label><Input
											id="degree_title"
											name="degree_title"
											value={profile.degree_title}
											placeholder="M.Kom / Dr."
										/>
									</div>
									<div class="space-y-2">
										<Label for="field_of_study">Bidang Studi</Label><Input
											id="field_of_study"
											name="field_of_study"
									...
									<div class="space-y-2">
										<Label for="education_grad_year">Tahun Lulus</Label><Input
											id="education_grad_year"
											name="education_grad_year"
											type="number"
											value={profile.education_grad_year}
										/>
									</div>
									<div class="space-y-2">
										<Label for="certification_number">No. Sertifikasi Dosen</Label><Input
								...
								<div class="flex justify-end gap-2">
									<Button
										type="button"
										variant="outline"
										onclick={() => (editLecturerEducation = false)}>Batal</Button
									>
									<Button type="submit">Simpan</Button>
								</div>
							</form>
			...
			<TabsContent value="expertise">
				<Card>
					<CardHeader>
						<CardTitle>Bidang Keahlian & Penelitian</CardTitle>
						<CardDescription>Kompetensi dan minat penelitian dosen</CardDescription>
					</CardHeader>
					<CardContent>
						<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
							<div>
							...
							<div>
								<p class="text-sm font-medium text-muted-foreground">Minat Penelitian</p>
								<p>{profile.research_interest || '-'}</p>
							</div>
							<div>
								<p class="text-sm font-medium text-muted-foreground">Google Scholar ID</p>
								<p>{profile.scholar_id || '-'}</p>
							</div>
							<div>
								<p class="text-sm font-medium text-muted-foreground">Scopus Author ID</p>
								<p>{profile.scopus_id || '-'}</p>
							</div>
							<div>
								<p class="text-sm font-medium text-muted-foreground">SINTA ID</p>
								<p>{profile.sinta_id || '-'}</p>
							</div>
							<div>
								<p class="text-sm font-medium text-muted-foreground">ORCID</p>
								<p>{profile.orcid || '-'}</p>
							</div>
						</div>
					</CardContent>
				...
				<Card>
					<CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
						<div class="space-y-1">
							...
							<CardDescription>Kontak dan alamat dosen</CardDescription>
						</div>
						{#if !editLecturerContact}
							<Button variant="ghost" size="sm" onclick={() => (editLecturerContact = true)}
								>Edit</Button
							>
						{/if}
					</CardHeader>
					<CardContent>
						{#if !editLecturerContact}
							<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
								...
								<div>
									...
									<p>{profile.phone || profile.phone_1 || '-'}</p>
								</div>
								<div>
						...
						{:else}
							<form
								...
								use:enhance={() => {
									return async ({ result }) => {
										if (result.type === 'success') editLecturerContact = false;
										await applyAction(result);
									};
								}}
								class="space-y-4"
							>
								<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
									...
									<div class="space-y-2">
										<Label for="phone">No. HP</Label><Input
											id="phone"
											name="phone"
											value={profile.phone || profile.phone_1}
										/>
									</div>
									<div class="space-y-2">
									...
									<div class="space-y-2 md:col-span-2">
										<Label for="address">Alamat</Label><Input
											id="address"
											name="address"
								...
								<div class="flex justify-end gap-2">
									<Button
										type="button"
										variant="outline"
										onclick={() => (editLecturerContact = false)}>Batal</Button
									>
									<Button type="submit">Simpan</Button>
								</div>
							</form>
			...
			<TabsContent value="citizenship">
				<Card>
					<CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
						...
						{#if !editCitizenship}
							<Button variant="ghost" size="sm" onclick={() => (editCitizenship = true)}
								>Edit</Button
							>
						{/if}
					</CardHeader>
