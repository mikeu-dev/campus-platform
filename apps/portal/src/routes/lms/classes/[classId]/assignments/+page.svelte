...
<div class="space-y-6">
	...
	<div class="grid gap-4">
		{#each data.assignments as item (item.id)}
			<a href="/lms/classes/{data.classId}/assignments/{item.id}" class="group block">
				<Card class="transition-all hover:border-primary/50 hover:shadow-md">
					<CardContent class="flex items-center justify-between p-6">
						<div class="min-w-0 flex-1">
							<div class="flex items-center justify-between">
								<h4
									class="text-lg font-semibold text-primary decoration-primary/30 underline-offset-4 group-hover:underline"
								>
									{item.title}
								</h4>
								<span
									class="ml-4 flex items-center rounded-md bg-secondary/50 px-2 py-1 text-sm text-muted-foreground"
								>
							...
							{#if item.description}
								<p class="mt-2 line-clamp-2 text-sm text-muted-foreground">{item.description}</p>
							{/if}
						</div>
