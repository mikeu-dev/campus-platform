...
export const load = async () => {
    ...
    try {
        const periodsRes = await axios.get(`${PUBLIC_ADMISSION_API_URL}/public/pmb/${tenantId}/periods`);
...
export const actions = {
    default: async ({ request }: any) => {
        ...
        try {
            const res = await axios.post(`${PUBLIC_ADMISSION_API_URL}/public/pmb/register`, registrationData);
            ...
            return fail(400, { message: error.response?.data?.message || 'Gagal mengirim pendaftaran. Silakan coba lagi.' });
        }
    }
};
