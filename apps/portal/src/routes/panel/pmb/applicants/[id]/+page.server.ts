...
export const actions = {
    updateStatus: async ({ request, params, locals }: any) => {
        ...
        try {
            await axios.put(`${PUBLIC_ADMISSION_API_URL}/admin/pmb/applicants/${id}/status`,
                { status, selection_score: score ? parseFloat(score) : null },
                { headers: { Authorization: `Bearer ${token}` } }
            );
            return { success: true };
        } catch (error: any) {
            return fail(400, { message: error.response?.data?.message || 'Gagal memperbarui status pendaftar' });
        }
    },
    verifyDocument: async ({ request, locals }: any) => {
        ...
        try {
            await axios.put(`${PUBLIC_ADMISSION_API_URL}/admin/pmb/documents/${docId}/status`,
                { status, notes },
                { headers: { Authorization: `Bearer ${token}` } }
            );
            return { success: true };
