...
<div class="space-y-6">
	<div class="flex items-center gap-4">
		...
		<div>
			<h2 class="text-3xl font-bold tracking-tight">Detail Mahasiswa</h2>
			<p class="text-muted-foreground">
				Kelola data mahasiswa: {profile.name} ({profile.platform_student_number})
			</p>
		</div>
	</div>
	...
	<Card>
		<CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
			...
			{#if !editAcademic}
				<Button variant="ghost" size="sm" onclick={() => (editAcademic = true)}>Edit</Button>
			{/if}
		</CardHeader>
						...
						<div class="space-y-4">
							<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
								...
								<div class="space-y-2">
									...
									<Input value={profile.current_semester} disabled class="bg-muted" />
								</div>
								<div class="space-y-2">
									<Label>Program Kelas</Label>
									<Input value={profile.class_program} disabled class="bg-muted" />
								</div>
								<div class="space-y-2">
									<Label>Tahun Masuk</Label>
									<Input value={profile.entry_year} disabled class="bg-muted" />
								</div>
								<div class="space-y-2">
									<Label>Jalur Pendaftaran</Label>
									<Input value={profile.entry_path} disabled class="bg-muted" />
								</div>
								<div class="space-y-2">
									<Label>Gelombang</Label>
									<Input value={profile.entry_batch} disabled class="bg-muted" />
								</div>
								<div class="space-y-2">
									<Label>Status</Label>
									<Input value={profile.status} disabled class="bg-muted capitalize" />
								</div>
								<div class="space-y-2">
									<Label>Group Akademik</Label>
									<Input value={profile.academic_group} disabled class="bg-muted" />
								</div>
								<div class="space-y-2">
									<Label>Pembimbing Akademik</Label>
									<Input value={profile.academic_advisor} disabled class="bg-muted" />
								</div>
							</div>
						</div>
					{:else}
						<form
							...
							use:enhance={() => {
								return async ({ result }) => {
									if (result.type === 'success') editAcademic = false;
									await applyAction(result);
								};
							}}
							class="space-y-4"
						>
							<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
								...
								<div class="space-y-2">
									<Label for="platform_student_number">NIM</Label>
									<Input
										id="platform_student_number"
										name="platform_student_number"
										value={profile.platform_student_number}
									/>
								</div>
								<div class="space-y-2">
									<Label for="study_program">Prodi</Label>
									<Input id="study_program" name="study_program" value={profile.study_program} />
								</div>
								<div class="space-y-2">
								...
								<div class="space-y-2">
									...
									<Input id="class_program" name="class_program" value={profile.class_program} />
								</div>
								<div class="space-y-2">
								...
								<div class="space-y-2">
									...
									<Input id="entry_path" name="entry_path" value={profile.entry_path} />
								</div>
								<div class="space-y-2">
									<Label for="entry_batch">Gelombang</Label>
									<Input id="entry_batch" name="entry_batch" value={profile.entry_batch} />
								</div>
								<div class="space-y-2">
								...
								<div class="space-y-2">
									...
									<Input id="academic_group" name="academic_group" value={profile.academic_group} />
								</div>
								<div class="space-y-2">
							...
							<div class="flex justify-end gap-2">
								<Button type="button" variant="outline" onclick={() => (editAcademic = false)}
									>Batal</Button
								>
				...
				<CardContent>
					{#if !editFather}
						<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
							...
							<div>
								<p class="text-sm font-medium text-muted-foreground">Tanggal Lahir</p>
								<p>{profile.father_date_of_birth || '-'}</p>
							</div>
							<div>
								<p class="text-sm font-medium text-muted-foreground">Agama</p>
				...
				<CardContent>
					{#if !editMother}
						<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
							...
							<div>
								<p class="text-sm font-medium text-muted-foreground">Tanggal Lahir</p>
								<p>{profile.mother_date_of_birth || '-'}</p>
							</div>
							<div>
								<p class="text-sm font-medium text-muted-foreground">Agama</p>
			...
			<Card>
				<CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
					<div class="space-y-1">
						...
						<CardDescription>Informasi mengenai sekolah asal.</CardDescription>
					</div>
					{#if !editSchool}
				...
				<CardContent>
					{#if !editSchool}
						<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
							...
							<div>
								<p class="text-sm font-medium text-muted-foreground">No. Ijazah</p>
						...
						>
							<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
								<div class="space-y-2 md:col-span-2">
									...
									<Input id="school_name" name="school_name" value={profile.school_name} />
								</div>
								<div class="space-y-2">
									<Label for="school_major">Jurusan</Label>
									<Input id="school_major" name="school_major" value={profile.school_major} />
								</div>
								<div class="space-y-2">
